...
import { getBestSellingProducts } from '../../redux/actions/user.actions'
import RecordNotFound from '../../components/RecordnotFound'
import AsyncStorage from '@react-native-community/async-storage'
import { _toast } from "../../constants/Index";
import Loader from '../../components/Loader'

const ViewAllBestProduct = ({ navigation, route }) => {
  const dispatch = useDispatch();
  const BestSellingProducts = useSelector((state) => state.userReducers.bestSellingProducts.data);
  const Refreshing = useSelector((state) => state.userReducers.bestSellingProducts.refreshing);
  const [index, setIndex] = React.useState(1);
  const [Selectedindex, setSelectedindex] = React.useState(null);
  // console.log('Selectedindex', Selectedindex)
  const [loading, setLoading] = useState([]);
  // console.log('ViewAllBestSellingProducts', BestSellingProducts)
  const update = (i, size) => {
    dispatch(getBestSellingProducts(1, 10,));
  };
  const onLoad = (i, size) => {
    dispatch(getBestSellingProducts(i, size,));
    setIndex(index + 1);
  };
  useEffect(() => {
    dispatch(getBestSellingProducts(1, 10,));
  ...
  const AdFavourites = async (item) => {
    ...
    try {
      ...
      if (res && res.success == true) {
        dispatch(getBestSellingProducts(1, 8,));
        setLoading(false);
      } else {
        setLoading(false);
      }
